...
export class RPCController {
  ...
  private activityThrottle = throttle(
    (isViewing?: boolean, isIdling?: boolean) =>
  ...
  private listen() {
    ...
    const gitListener = dataClass.onUpdate(() =>
    ...
    if (config.get(CONFIG_KEYS.Status.Problems.Enabled))
      this.listeners.push(diagnosticsChange);
    if (config.get(CONFIG_KEYS.Status.Idle.Check))
      this.listeners.push(changeWindowState);
  ...
  private checkCanSend(isIdling: boolean): boolean {
    ...
    if (isIdling && config.get(CONFIG_KEYS.Status.Idle.DisconnectOnIdle))
      return (this.canSendActivity = false);
    ...
    if (whitelistEnabled) {
      ...
      else if (!whitelist!.includes(userId))
        return (this.canSendActivity = false);
    }
  ...
  async enable() {
    ...
    this.iconTimeout = setTimeout(
      () => (editor.statusBarItem.text = "$(smiley)"),
      5000,
    );
  }
