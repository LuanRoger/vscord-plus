...
export class Data implements DisposableLike {
  ...
  public constructor(debug = false) {
    ...
    this.rootListeners.push(
      ...
      window.onDidChangeActiveTextEditor((e) => {
        ...
        if (e)
          this.debug(
            `root(): window.onDidChangeActiveTextEditor: got URI '${e.document.uri.scheme}'`,
          );
  ...
  public get fileName(): string | undefined {
    const _file = this.editor
      ? parse(this.editor.document.uri.fsPath)
      : undefined;
  ...
  public get folderAndFile(): string | undefined {
    ...
    if (
      ...
      directory === this.workspaceFolder?.name
    )
      return file;
  ...
  public get workspaceName(): string | undefined {
    ...
    if (this.editor?.document.uri.scheme === "vscode-remote")
  ...
  public get gitRemoteUrl(): gitUrlParse.GitUrl | undefined {
    const v = stripCredential(
      this._remote?.fetchUrl ?? this._remote?.pushUrl ?? "",
    );
