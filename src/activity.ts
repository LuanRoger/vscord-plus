...
export const activity = async (
  ...
  if (config.get(CONFIG_KEYS.Status.ShowElapsedTime)) {
    presence.startTimestamp = config.get(
      CONFIG_KEYS.Status.ResetElapsedTimePerFile,
    )
      ? Date.now()
  ...
  const detailsIdleEnabled = config.get(
    CONFIG_KEYS.Status.Details.Idle.Enabled,
  );
  ...
  const gitRepo = dataClass.gitRemoteUrl
    ...
    .replace(/\.git$/, "");
  const gitOrg =
    dataClass.gitRemoteUrl?.organization ?? dataClass.gitRemoteUrl?.owner;
  const gitHost = dataClass.gitRemoteUrl?.source;

  const isRepositoryExcluded =
    !!gitRepo &&
    isExcluded(config.get(CONFIG_KEYS.Ignore.Repositories)!, gitRepo);
  const isOrganizationExcluded =
    !!gitOrg &&
    isExcluded(config.get(CONFIG_KEYS.Ignore.Organizations)!, gitOrg);
  const isGitHostExcluded =
    !!gitHost && isExcluded(config.get(CONFIG_KEYS.Ignore.GitHosts)!, gitHost);
  const isGitExcluded =
  ...
  if (!isWorkspaceExcluded)
    isWorkspaceExcluded =
      ...
      isExcluded(
        config.get(CONFIG_KEYS.Ignore.Workspaces)!,
        dataClass.workspaceName,
      );

  const isNotInFile = !isWorkspaceExcluded && !dataClass.editor;

  const isDebugging =
    config.get(CONFIG_KEYS.Status.State.Debugging.Enabled) &&
  ...
  const PROBLEMS = config.get(CONFIG_KEYS.Status.Problems.Enabled)
    ? await replaceFileInfo(
        replaceGitInfo(
          replaceAppInfo(config.get(CONFIG_KEYS.Status.Problems.Text)!),
          isGitExcluded,
        ),
        isWorkspaceExcluded,
  ...
  switch (status) {
    case CURRENT_STATUS.IDLE: {
      if (!isWorkspaceExcluded) {
        if (detailsIdleEnabled && detailsEnabled)
          details = await replaceAllText(
            config.get(CONFIG_KEYS.Status.Details.Text.Idle)!,
          );
        if (stateIdleEnabled && stateEnabled)
          state = await replaceAllText(
            config.get(CONFIG_KEYS.Status.State.Text.Idle)!,
          );
      }

      largeImageKey = await replaceAllText(
        config.get(CONFIG_KEYS.Status.Image.Large.Idle.Key)!,
      );
      largeImageText = await replaceAllText(
        config.get(CONFIG_KEYS.Status.Image.Large.Idle.Text)!,
      );
      smallImageKey = await replaceAllText(
        config.get(CONFIG_KEYS.Status.Image.Small.Idle.Key)!,
      );
      smallImageText = await replaceAllText(
    ...
    case CURRENT_STATUS.NOT_IN_FILE: {
      if (detailsEnabled)
        details = await replaceAllText(
          config.get(CONFIG_KEYS.Status.Details.Text.NotInFile)!,
        );
      if (stateEnabled)
        state = await replaceAllText(
          config.get(CONFIG_KEYS.Status.State.Text.NotInFile)!,
        );

      largeImageKey = await replaceAllText(
        config.get(CONFIG_KEYS.Status.Image.Large.NotInFile.Key)!,
      );
      largeImageText = await replaceAllText(
        config.get(CONFIG_KEYS.Status.Image.Large.NotInFile.Text)!,
      );

      smallImageKey = await replaceAllText(
        config.get(CONFIG_KEYS.Status.Image.Small.NotInFile.Key)!,
      );
      smallImageText = await replaceAllText(
        config.get(CONFIG_KEYS.Status.Image.Small.NotInFile.Text)!,
      );
      break;
    }
  }
  let buttons = await getPresenceButtons(
    isIdling,
    isGitExcluded,
    status,
    replaceAllText,
  ...
  // Clean up
  if (!presence.details || presence.details.trim() === "")
    delete presence.details;
  if (!presence.state || presence.state.trim() === "") delete presence.state;
  if (!presence.largeImageKey || presence.largeImageKey.trim() === "")
    delete presence.largeImageKey;
  if (!presence.largeImageText || presence.largeImageText.trim() === "")
...
async function createButton(
  ...
  if (
    !config.get(
      isGit && state != "Inactive"
        ? currentState.Git[state].Enabled
...
export const getPresenceButtons = async (
  ...
  return [button1.button, button2.button].filter(
    Boolean,
  ) as GatewayActivityButton[];
};
...
export const replaceAppInfo = (text: string): string => {
  ...
  const isCodium =
    appName.startsWith("VSCodium") || appName.startsWith("codium");
...
export const getTotalProblems = (
  countedSeverities: PROBLEM_LEVEL[],
): number => {
  let totalProblems = 0;
...
export const replaceGitInfo = (text: string, excluded = false): string => {
  ...
  const replaceMap = new Map([
    [
      "{git_owner}",
      (!excluded ? dataClass.gitRemoteUrl?.owner : undefined) ?? FAKE_EMPTY,
    ],
    [
      "{git_provider}",
      (!excluded ? dataClass.gitRemoteUrl?.source : undefined) ?? FAKE_EMPTY,
    ],
    [
      "{git_repo}",
      (!excluded
        ? (dataClass.gitRemoteUrl?.name ?? dataClass.gitRepoName)
        : undefined) ?? FAKE_EMPTY,
    ],
    [
      "{git_branch}",
      (!excluded ? dataClass.gitBranchName : undefined) ?? FAKE_EMPTY,
    ],
    [
      "{git_url}",
      (!excluded
        ? (dataClass.gitRemoteUrl?.toString("https") ?? "").replace(
            /\.git$/,
            "",
          )
        : undefined) ?? FAKE_EMPTY,
...
export const replaceFileInfo = async (
  ...
  const fileIcon = dataClass.editor
    ? resolveLangName(dataClass.editor.document)
    : "text";
  ...
  if (dataClass.editor && dataClass.workspaceName && !excluded) {
    ...
    relativeFilepath = workspace.asRelativePath(
      dataClass.editor.document.fileName,
    );
    const relativePath = workspace
      .asRelativePath(dataClass.editor.document.fileName)
      .split(sep);
  ...
  const replaceMap = new Map([
    ...
    [
      ...
      config.get(CONFIG_KEYS.Status.Problems.Enabled)
        ? getTotalProblems(
            config.get(CONFIG_KEYS.Status.Problems.countedSeverities)!,
          ).toLocaleString()
        : FAKE_EMPTY,
    ],
    ...
    [
      "{current_line}",
      selection ? (selection.active.line + 1).toLocaleString() : FAKE_EMPTY,
    ],
    [
      "{current_column}",
      selection
