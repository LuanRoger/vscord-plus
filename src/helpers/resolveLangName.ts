...
export const toLower = <S extends string>(str: S) =>
  str.toLocaleLowerCase() as Lowercase<S>;
export const toUpper = <S extends string>(str: S) =>
  str.toLocaleUpperCase() as Uppercase<S>;
...
export const resolveLangName = (document: TextDocument): string => {
  ...
  const findKnownExtension =
    Object.keys(ADDITIONAL_FILE_MAPPING).find((extension) =>
      fileExtensionEquals(filename, extension),
    ) ??
    Object.keys(KNOWN_EXTENSIONS).find((extension) =>
      fileExtensionEquals(filename, extension),
    );

  const areLanguagesPrioritized = config.get(
    CONFIG_KEYS.Behaviour.PrioritizeLanguagesOverExtensions,
  );
  ...
  const knownExtension = findKnownExtension
    ? (ADDITIONAL_FILE_MAPPING[findKnownExtension] ??
      KNOWN_EXTENSIONS[findKnownExtension])
    : findKnownLanguage?.image;

  const knownLanguage = findKnownLanguage
    ? findKnownLanguage.image
    : knownExtension;
