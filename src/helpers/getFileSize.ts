...
export const getFileSize = async (
  ...
  let round = 2;
  if (
    config.get(CONFIG_KEYS.File.Size.Round) === 0 ||
    config.get(CONFIG_KEYS.File.Size.Round)
  )
    round = config.get(CONFIG_KEYS.File.Size.Round)!;

  let spacer = " ";
  if (
    config.get(CONFIG_KEYS.File.Size.Spacer) === "" ||
    config.get(CONFIG_KEYS.File.Size.Spacer)
  )
    spacer = config.get(CONFIG_KEYS.File.Size.Spacer)!;

  const fileSizeStandard: FileSizeStandard =
    config.get(CONFIG_KEYS.File.Size.Standard) ?? "iec";
