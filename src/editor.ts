...
class EditorController {
  ...
  toggleStatusBarAlignment(onLeft?: boolean) {
    ...
    const alignment =
      (onLeft ?? config.get(cfgKey) === "Right") ? "Left" : "Right";

    config.update(
      cfgKey,
      alignment satisfies ExtensionConfigurationType[typeof cfgKey],
    );
    return alignment;
  }
  updateStatusBarFromConfig() {
    ...
    if (editor.statusBarItem.alignment !== alignment) {
      ...
      // copy
      editor.statusBarItem.accessibilityInformation =
        old.accessibilityInformation;
