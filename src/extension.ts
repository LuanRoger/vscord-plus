...
export const registerListeners = (ctx: ExtensionContext) => {
  const onConfigurationChanged = workspace.onDidChangeConfiguration(
    async () => {
...
export const registerCommands = (ctx: ExtensionContext) => {
  ...
  const enableCommand = commands.registerCommand(
    "vscord-plus.enable",
    async () => {
      await disable(false);
      await enable(false);
  ...
  const disableCommand = commands.registerCommand(
    "vscord-plus.disable",
    async () => {
  ...
  const enableWorkspaceCommand = commands.registerCommand(
    "vscord-plus.workspace.enable",
    async () => {
      logInfo("Enabled Discord Rich Presence");
  ...
  const disableWorkspaceCommand = commands.registerCommand(
    "vscord-plus.workspace.disable",
    async () => {
      logInfo("Disabled Discord Rich Presence");
  ...
  const reconnectCommand = commands.registerCommand(
    "vscord-plus.reconnect",
    async () => {
      logInfo("Reconnecting to Discord Gateway...");

      editor.statusBarItem.text =
        "$(search-refresh) Connecting to Discord Gateway...";
      ...
      await controller
        ...
        .catch(() => {
          ...
          editor.statusBarItem.text =
            "$(search-refresh) Reconnect to Discord Gateway";
  ...
  const enablePrivacyModeCommand = commands.registerCommand(
    "vscord-plus.enablePrivacyMode",
    async () => {
  ...
  const disablePrivacyModeCommand = commands.registerCommand(
    "vscord-plus.disablePrivacyMode",
    async () => {
